...
class Cadisv2(BaseDataset):
    def __init__(self,
        ...
        super(Cadisv2, self).__init__(ignore_label, base_size,
    ...
    def __getitem__(self, index):
        ...
        name = item["name"]
        image_path = os.path.join(self.root, 'CaDISv2', item['img'])
        label_path = os.path.join(self.root, 'CaDISv2', item['label'])
        image = cv2.imread(
            image_path,
            cv2.IMREAD_COLOR
        )
